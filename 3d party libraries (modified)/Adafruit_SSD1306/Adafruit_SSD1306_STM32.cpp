...
//*************SH1106**********************
void Adafruit_SSD1306::display_alt(void) {
	
    ssd1306_command(SSD1306_SETSTARTLINE | 0); // line #0
    
    for (int8_t i =0 ; i <8; i++)
    {
        ssd1306_command(0xB0 + i);		// Set row
        ssd1306_command(SSD1306_SETLOWCOLUMN+2); // Set lower column address +2 (shift right!)
...
//*************SH1106 page********************
void Adafruit_SSD1306::display_alt(uint8_t startpage, uint8_t endpage) {

    ssd1306_command(SSD1306_SETSTARTLINE | 0); // line #0

    for (int8_t i =startpage ; i <endpage+1; i++)
    {
        ssd1306_command(0xB0 + i);		// Set row
        ssd1306_command(SSD1306_SETLOWCOLUMN+2); // Set lower column address +2 (shift right!)
        ssd1306_command(SSD1306_SETHIGHCOLUMN); // Set higher column address
...
// partial update of display  void display(uint8_t startpage, uint8_t endpage); only I2C

void Adafruit_SSD1306::display(uint8_t startpage, uint8_t endpage) {
  ...
  ssd1306_command(endpage); // Page end address

	  // Gesamt: 128x64 = 8192 Punkte = 1024KB
		//8 Pages zu: 128 Bytes
		//I2C: nur 32 Bytes pro Vorgang = 4x pro Page!
 
  Wire.beginTransmission(_i2caddr);
      WIRE_WRITE(0x40);
	  int endpoint = (endpage+1)*128;
	  byte counter=0;
	  for (int b=startpage*128; b<endpoint; b++)	
	  {
		 byte aa=buffer[b];
           WIRE_WRITE(buffer[b]);
			counter++;
