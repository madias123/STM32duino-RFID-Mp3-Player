...
boolean Adafruit_VS1053_FilePlayer::useInterrupt(uint8_t type) {
  ...
  if (type == VS1053_FILEPLAYER_PIN_INT) {
	...
	#pragma message "attachInterrupt"
    return true;
  }
  return false;
}
...
boolean Adafruit_VS1053_FilePlayer::startPlayingFile(const char *trackname) {
  ...
  currentTrack = audio_SD.open(trackname);
  if (!currentTrack) {
...
// ##############################################
//******new function: call track ID****
boolean Adafruit_VS1053_FilePlayer::startPlayingFile(const char *dirname, uint8_t tracknumber) {
  // reset playback
  sciWrite(VS1053_REG_MODE, VS1053_MODE_SM_LINE1 | VS1053_MODE_SM_SDINEW);
  // resync
  sciWrite(VS1053_REG_WRAMADDR, 0x1e29);
  sciWrite(VS1053_REG_WRAM, 0);
  if (!dirFileVS.open(dirname, O_READ)) {
    Serial.println("BAD DIR!") ;
  }
  else 
	  Serial.println("good DIR!") ;
  
  

  
  if (!currentTrack.open(&dirFileVS, tracknumber, O_READ)){
    Serial.println("BAD FILENAME!") ;
	return false;
  }
  else 
	  Serial.println("good FILENAME!") ;
...
uint16_t Adafruit_VS1053::loadPlugin(char *plugname) {
  ...
  File plugin = audio_SD.open(plugname);
  if (!plugin) {
